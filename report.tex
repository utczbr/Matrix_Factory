...
\paragraph{Full-horizon batch.} At a full 60-minute simulated horizon, the recovery effects of ADACOR's spike-handling are fully realised. Table~\ref{tab:mc-very-long} shows the results for 15 runs per schema.
...
\midrule
PROSA & $0.0109 \pm 0.0045$ & $262.6 \pm 157.3$ & $1944.7$ & $3.26 \pm 1.91$ & $0.0028$ & $450$ of $523$ \\
ADACOR & $0.0131 \pm 0.0019$ & $288.6 \pm 140.6$ & $2340.3$ & $3.57 \pm 1.61$ & $0.0028$ & $448$ of $519$ \\
\bottomrule
\end{tabular}
...
Figure~\ref{fig:mc-bars} visualises the two headline metrics across all three horizons, making the directional reversal between short- and long-horizon behaviour immediately visible.
...
\addplot[fill=black!25, draw=black!70] coordinates {(Short,0.0139) (Long,0.0123) (Full,0.0109)};
\addplot[fill=black!60, draw=black!90] coordinates {(Short,0.0116) (Long,0.0134) (Full,0.0131)};
\legend{PROSA,ADACOR}
\end{axis}
\end{tikzpicture}%
\hfill
...
\addplot[fill=black!25, draw=black!70] coordinates {(Short,145.0) (Long,298.7) (Full,262.6)};
\addplot[fill=black!60, draw=black!90] coordinates {(Short,169.0) (Long,314.1) (Full,288.6)};
\legend{PROSA,ADACOR}
\end{axis}
\end{tikzpicture}
\caption{Grouped bar comparison of throughput and mean tardiness for PROSA and ADACOR across the three batch horizons. ADACOR's throughput disadvantage in the short batch reverses in the longer batches, while its mean tardiness remains comparable despite holding orders back during the spike.}
\label{fig:mc-bars}
\end{figure*}

\paragraph{Statistical tests.} Per-metric group comparisons (Shapiro--Wilk normality check, then Welch's $t$-test where both groups pass at $\alpha=0.05$, otherwise Mann--Whitney $U$) are summarised in Table~\ref{tab:mc-sig}; at these sample sizes none of the three headline metrics reach conventional significance in either batch.
...
3600-tick ($n{=}15$) & Throughput & Mann--Whitney $U$ & $0.507$ & Not significant \\
3600-tick ($n{=}15$) & Tardiness mean & Mann--Whitney $U$ & $0.619$ & Not significant \\
3600-tick ($n{=}15$) & WIP mean & Mann--Whitney $U$ & $0.455$ & Not significant \\
\bottomrule
\end{tabular}
