...
class LUTManager(Component):
    ...
    def _generate_lut(self, fluid: str) -> Dict[PropertyType, npt.NDArray]:
        ...
        Uses POSIX advisory file locking (``fcntl.flock(LOCK_EX)``) to
        prevent race conditions when multiple daemons start concurrently
        and all attempt to generate the same LUT cache (doc2 §4.2).
        ...
        # POSIX advisory lock: 30-daemon contention safety (doc2 §4.2)
        lock_path = self.config.cache_dir / f".{fluid}_generate.lock"
        lock_path.touch(exist_ok=True)
        with open(lock_path, 'w') as lock_fd:
            logger.info(f"Acquiring generation lock for {fluid}...")
            fcntl.flock(lock_fd.fileno(), fcntl.LOCK_EX)
            logger.info(f"Lock acquired for {fluid}. Generating LUT...")

            try:
                # Double-check: another process may have generated while we waited
                cache_path = self._get_cache_path(fluid)
                if cache_path.exists():
                    logger.info(f"Cache appeared while waiting on lock for {fluid}. Loading.")
                    try:
                        return self._load_from_cache(cache_path)
                    except Exception:
                        logger.warning(f"Stale cache for {fluid}. Regenerating.")

                lut = self._generate_lut_unlocked(fluid)
                self._save_to_cache(fluid, cache_path)
                return lut
            finally:
                fcntl.flock(lock_fd.fileno(), fcntl.LOCK_UN)

    def _generate_lut_unlocked(self, fluid: str) -> Dict[PropertyType, npt.NDArray]:
        """
        Inner LUT generation logic (called under lock).
