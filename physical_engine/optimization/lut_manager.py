...
from physical_engine.core.constants import StandardConditions
...
try:
    from physical_engine.optimization.numba_ops import (
...
class LUTConfig:
    ...
    cache_dir: Path = Path(__file__).resolve().parents[2] / '.physical_engine' / 'lut_cache'


MATRIX_FACTORY_LUT_CONFIG = LUTConfig(
    pressure_min=0.5e5, pressure_max=450e5, pressure_points=150,
    temperature_min=250.0, temperature_max=400.0, temperature_points=150,
    entropy_points=100,
    properties=('D', 'H', 'S', 'C'),
    fluids=('H2',),
)
...
class LUTManager(Component):
    ...
    def initialize(self, dt: float = 0.0, registry: Optional['ComponentRegistry'] = None) -> None:
        ...
        # SLOW PATH — only one process may generate the shared LUT at a time.
        lock_path = self.config.cache_dir / ".lut_generation.lock"
        with open(lock_path, "w") as lockfile:
            fcntl.flock(lockfile.fileno(), fcntl.LOCK_EX)
            try:
                # Re-check: a sibling daemon may have finished generating while we waited.
                if stacked_cache_path.exists() and self._load_stacked_cache(stacked_cache_path):
                    self._generate_saturation_lut()
                    logger.info("LUT Manager initialization complete (fast path, post-lock)")
                    return
    ...
    def lookup_batch(
        ...
        from physical_engine.optimization.numba_ops import batch_bilinear_interp_jit
    ...
    def lookup_mixture_enthalpy(
        ...
        from physical_engine.optimization.numba_ops import get_mixture_enthalpy_fast
    ...
    def lookup_isentropic_enthalpy(
        ...
        from physical_engine.optimization.numba_ops import bilinear_interp_jit
    ...
    def lookup_pressure_from_density(
        self, fluid: str, rho_target_kg_m3: float, temperature_k: float,
        tol_rel: float = 1e-4, max_iter: int = 30,
    ) -> float:
        """Invert the (P,T)->D LUT via bisection to answer 'what pressure gives
        this density at this temperature' — needed by TankArray, not provided
        by the forward-only lookup() API."""
        if not self._initialized:
            self.initialize()
        p_lo, p_hi = self._p_min, self._p_max
        d_lo = self.lookup(fluid, 'D', p_lo, temperature_k)
        d_hi = self.lookup(fluid, 'D', p_hi, temperature_k)
        if rho_target_kg_m3 <= d_lo:
            return p_lo
        if rho_target_kg_m3 >= d_hi:
        ...
        for _ in range(max_iter):
            ...
            if abs(d_mid - rho_target_kg_m3) / rho_target_kg_m3 < tol_rel:
                return p_mid
            if d_mid < rho_target_kg_m3:
                p_lo = p_mid
            else:
                p_hi = p_mid
    ...
    def _generate_lut(self, fluid: str) -> Dict[PropertyType, npt.NDArray]:
        ...
        for prop in self.config.properties:
            ...
            from physical_engine.core.exceptions import ComponentInitializationError
            for i, pressure in enumerate(self._pressure_grid):
                for j, temperature in enumerate(self._temperature_grid):
                    try:
                        table[i, j] = CP.PropsSI(prop, 'P', pressure, 'T', temperature, fluid)
                    except (ValueError, IndexError):
                        # Expected: query point outside this fluid's valid phase domain.
                        table[i, j] = np.nan
                    except Exception as e:
                        raise ComponentInitializationError(
                            f"Unexpected CoolProp failure generating {fluid} {prop} LUT at "
                            f"P={pressure/1e5:.1f} bar, T={temperature:.1f} K: {e}"
                        ) from e
        ...
        for i, p in enumerate(self._pressure_grid):
            for j, s in enumerate(self._entropy_grid):
                try:
                    ...
                    from physical_engine.core.exceptions import ComponentInitializationError
                    raise ComponentInitializationError(
                        f"Unexpected CoolProp failure generating {fluid} H(P,S) LUT at "
                        f"P={p/1e5:.1f} bar, S={s:.1f}: {e}"
                    ) from e
