...
from physical_engine.core.component import Component
from physical_engine.core.stream import Stream
from physical_engine.core.constants import GasConstants
from physical_engine.models.thermal_inertia import ThermalInertiaModel
from physical_engine.models.flow_dynamics import PumpFlowDynamics
from physical_engine.optimization.coolprop_lut import CoolPropLUT
from physical_engine.optimization import numba_ops
import math

# Import mixture thermodynamics for rigorous enthalpy calculations
try:
    from physical_engine.optimization import mixture_thermodynamics as mix_thermo
...
class Chiller(Component):
        ...
        if use_mix_thermo:
            try:
                ...
                if (lut_manager is not None and 
                    ...
                    from physical_engine.optimization import numba_ops # Ensure imported
    ...
    def step(self, t: float) -> None:
        ...
        # NOTE: max_Q_W bounds Q_dot_W here, which at this point in the
        # method is SENSIBLE heat only (computed against a vapor-phase
        # target_stream). self.cooling_load_kw is increased further below
        # by self.latent_heat_kw once condensation is evaluated — so the
        # final cooling_load_kw / electrical_power_kw / heat_rejected_kw
        # can end up ABOVE nameplate capacity even though this check passed.
        # A fully correct fix requires iterating final_temp_k until
        # sensible + latent load together equal max_Q_W, which needs a real
        # CoolProp/LUT environment to validate and is out of scope for this
        # patch. The check added after the latent-heat update below at
        # least makes the violation visible in logs instead of silent.
        ...
        # FIX (visibility, not a full re-solve — see NOTE above the sensible
        # capacity check): the sensible-only cap applied earlier does not
        # bound this corrected total. Surface it instead of reporting a
        # silently-over-nameplate load.
        max_total_Q_kw = self.cooling_capacity_kw * self.efficiency
        if abs(self.cooling_load_kw) > max_total_Q_kw * 1.001:  # small tolerance for float noise
            self.logger.warning(
                f"Chiller {self.component_id}: TOTAL load (sensible {self.sensible_heat_kw:.1f} kW "
                f"+ latent {self.latent_heat_kw:.1f} kW = {self.cooling_load_kw:.1f} kW) exceeds "
                f"nameplate capacity {self.cooling_capacity_kw:.1f} kW after condensation. "
                "The sensible-only cap above does not account for latent heat; outlet "
                "temperature/electrical power below are computed from this uncapped total."
            )
