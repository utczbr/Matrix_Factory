...
class Chiller(Component):
    ...
    def __init__(
        ...
        # PEMFC stack thermal coupling (doc2 §4.4)
        # Q_input accumulates heat from StackThermalModel.step() output
        self._Q_input_W: float = 0.0
    ...
    def step(self, t: float) -> None:
        ...
        # Consume stack heat input (PEMFC thermal coupling)
        Q_stack_W = self._Q_input_W
        self._Q_input_W = 0.0  # Reset after consumption
        ...
        # Include external stack heat load (PEMFC thermal coupling, doc2 §4.4)
        Q_dot_W += Q_stack_W
    ...
    def receive_stack_heat(self, Q_output_W: float) -> None:
        """Accept heat load from the PEMFC stack thermal model.

        This is the coupling interface described in doc2 §4.4:
        ``StackThermalModel.step()`` returns ``Q_output`` which the
        orchestrator passes to this method *before* the chiller's own
        ``step()`` runs.

        Args:
            Q_output_W: Heat transferred from the stack skin to the
                        coolant loop [W].  Positive means heat *into*
                        the chiller (additional cooling demand).
        """
        self._Q_input_W += Q_output_W
