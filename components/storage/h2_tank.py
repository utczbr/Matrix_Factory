...
class TankArray(Component):
    ...
    def __init__(
        ...
        # Thread safety for Numba nogil=True in-place mutations (doc2 §4.1)
        # Prevents torn reads when concurrent RunBatchTest snapshots state
    ...
    def step(self, t: float) -> None:
        ...
        Wrapped in _state_lock to prevent torn reads from concurrent threads
        (Numba @njit(nogil=True) kernels mutate arrays in-place with GIL released).
