...
### Step 2: Run Station 3 & 4 Physics Calibration & Unit Tests

```bash
pytest physical_engine/factory_simulation/ -v
```
*Output:* Validates Cockcroft–Latham damage threshold $C_{\mathrm{crit,NCL}} = 0.35$, Archard wear laws, and Station 4 non-linear GDL compression models.
...
```
*Output:* Executes comparative co-simulation runs under energy price spike profiles and outputs `analysis/prosa_vs_adacor_summary.csv`.
